...
bool pa_card_profile_contains_type_ports(pa_card_profile *c, pa_direction_t direction, pa_available_t type) {
    pa_card *card;
    pa_device_port *port;
    void *state;
    ...
    PA_HASHMAP_FOREACH(port, card->ports, state) {
        if (pa_hashmap_get(port->profiles, c->name) && port->available == type && port->direction == direction)
            return true;
    }
...
void pa_card_choose_initial_profile(pa_card *card) {
    ...
    PA_HASHMAP_FOREACH(profile, card->profiles, state) {
        pa_log_debug("%s profile %s availability status %s", card->name, profile->name,
                     profile->available == PA_AVAILABLE_YES ? "yes" : profile->available == PA_AVAILABLE_NO ? "no" : "unknown");
    ...
    if (!best) {
        pa_log_debug("No profile found with availability status 'yes' or 'unknown' for card %s", card->name);
        PA_HASHMAP_FOREACH(profile, card->profiles, state) {
            if (!best || profile->priority > best->priority)
                best = profile;
        }
